...
class RateLimiter:
    ...
    def __init__(
        ...
        if backend == "redis":
            ...
            self._session_turns: dict = defaultdict(int)    # session_id → message count
            self._user_sessions: dict = defaultdict(dict)   # user_id → {date_str: count}
            self._llm_calls: dict = defaultdict(int)        # session_id → LLM call count
            # FIX: track which sessions have been registered for daily counting
            # separately from _session_turns so "new session" detection stays
            # correct after the first message is sent.
            self._seen_sessions: set = set()                # session_ids registered today
    ...
    def _check_local(self, user_id: str, session_id: str) -> None:
        with self._lock:
            ...
            # sessions_per_user_per_day — only count each session_id once.
            # FIX: use _seen_sessions set to track whether this session has
            # already been registered.  Previously used `session_id not in
            # _session_turns` which broke after the first increment_message()
            # call because the key was then present with value 1.
            if session_id not in self._seen_sessions:
                user_today = self._user_sessions[user_id].get(today, 0)
                if user_today >= self.config.sessions_per_user_per_day:
                    raise RateLimitExceeded(
