...
# Provisions all doc-upload-lambda infrastructure for an environment.
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  🏗️   Doc Upload Lambda — Constructor"
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
  ...
  echo "    export TF_VAR_auth_token='7KmP@9xQ2NvL5!'"
  echo "    export TF_VAR_openai_api_key='sk-proj-...'"
  echo "    export TF_VAR_pdf_api_key='...'"
...
PLAN_FILE="/tmp/doc-upload-${ENV}.tfplan"
echo "📋  Planning..."
terraform plan \
  -var-file="${TFVARS}" \
  -out="${PLAN_FILE}" \
...
echo ""
echo "  Next: build & push your Docker image"
